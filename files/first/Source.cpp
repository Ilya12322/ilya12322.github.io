...
void answer(
	double r_1, double t_1, double s_1,
	double r_10, double r_20, double t_10, double t_20, double s_10, double s_20);

double a_fault(double answer) {
	double a_answer = 4.2;
	return abs(answer - a_answer);
}
double i_fault(double answer) { return abs(a_fault(answer) / answer); }

double a_Runge(double answer_1, double answer_2) {
	int k = 4;
	double n = 1;
	for (int i = 0; i < k; ++i)
		n *= 2;

	return (answer_1 - answer_2) / (n - 1);
}
...
int main() {
	...
	double a_1 = 1, a_2 = -2.6;
	double b_1 = 2, b_2 = -1.6;
	...
	answer(an_rec_1, an_tr_1, an_sym_1,
...
void answer(
	...
	fout << "absolute fault: " << abs(a_Runge(r_10, r_20)) << "\n";
	...
	fout << "absolute fault: " << abs(a_Runge(t_10, t_20)) << "\n";
	...
	fout << "absolute fault: " << abs(a_Runge(s_10, s_20)) << "\n";
